...
function AboutInner() {
  ...
  useEffect(() => {
    const q = getTabValue(params.get('tab'));
    if (q !== tab) setTab(q);
  }, [params, tab]);
      ...
      <div className="flex gap-1 p-1 bg-slate-100 rounded-xl mb-6 overflow-x-auto">
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex items-center gap-1.5 flex-1 justify-center py-2 px-3 rounded-lg text-sm font-semibold transition-all whitespace-nowrap ${
      ...
      {loading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-5">
          ...
          {tab === 'department' && (<>
            ...
            <div>
              ...
              <textarea rows={7} value={form.aboutIntro ?? ''} onChange={e => F('aboutIntro', e.target.value)}
                placeholder="The Department of Computer Science and Engineering at GSTU…" className={taCls}/>
            </div>
            <div>
          ...
          {tab === 'history' && (<>
            ...
            <div>
              <label className={lCls}>Department History</label>
              <textarea rows={10} value={(form as unknown as Record<string,string>).aboutHistory ?? ''} onChange={e => F('aboutHistory' as keyof SiteSettings, e.target.value)}
                placeholder="The department was founded in 2011…" className={taCls}/>
            </div>
          </>)}
          ...
          {tab === 'chairman' && (<>
            <p className={sec}>Chairman&apos;s Message — homepage + /about/chairman page</p>
            ...
            <div>
              <label className={lCls}>Message <span className="text-slate-400 font-normal">(separate paragraphs with blank line)</span></label>
              <textarea rows={10} value={form.chairmanMessage ?? ''} onChange={e => F('chairmanMessage', e.target.value)}
                placeholder="Welcome to the Department…" className={taCls}/>
            </div>
          </>)}
